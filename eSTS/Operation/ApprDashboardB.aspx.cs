...
{
    public partial class ApprDashboardB : System.Web.UI.Page
    {
        ...
        protected void lilView_Init(object sender, EventArgs e)
        {
            ...
            contentUrl = string.Format("OpAppB.aspx?mode=v&appr=1&adm=0&sno={0}", DataBinder.Eval(container.DataItem, "OperationAppID"));
        ...
        protected void lilPayment_Init(object sender, EventArgs e)
        {
            ...
            contentUrl = string.Format("JLMPayment.aspx?mode=e&method=b&appr=1&adm=0&sno={0}", DataBinder.Eval(container.DataItem, "OperationAppID"));
        ...
        protected void lilCM_Init(object sender, EventArgs e)
        {
            ...
            contentUrl = string.Format("UploadCMBL_B.aspx?mode=v&m=b&appr=1&adm=0&sno={0}", DataBinder.Eval(container.DataItem, "OperationAppID"));

            link.EncodeHtml = false;
            if (DataBinder.Eval(container.DataItem, "FlowActionStatusID").ToString() == "7f99dadd-6895-4091-a5f3-c2803acfff1f")
            {
                link.NavigateUrl = contentUrl;// "javascript:void(0);";
        ...
        protected void lilCancel_Init(object sender, EventArgs e)
        {
            ...
            GridViewDataItemTemplateContainer container = (GridViewDataItemTemplateContainer)link.NamingContainer;
            contentUrl = string.Format("OpAppB.aspx?mode=c&appr=1&adm=0&sno={0}", DataBinder.Eval(container.DataItem, "OperationAppID"));
        ...
        protected void grid_HtmlDataCellPrepared(object sender, DevExpress.Web.Bootstrap.BootstrapGridViewTableDataCellEventArgs e)
        {
            try
            {
                ...
                if (e.DataColumn.FieldName == "VSName")
                {
                    ...
                    string latDegree = gv.GetRowValues(e.VisibleIndex, "VSLatDegree").ToString();
                    string latMin = gv.GetRowValues(e.VisibleIndex, "VSLatMin").ToString();
                    string longDegree = gv.GetRowValues(e.VisibleIndex, "VSLongDegree").ToString();
                    string longMin = gv.GetRowValues(e.VisibleIndex, "VSLongMin").ToString();
                    string latitude = gv.GetRowValues(e.VisibleIndex, "VSLatitude").ToString();
                    string longitude = gv.GetRowValues(e.VisibleIndex, "VSLongitude").ToString();
            ...
            try
            {
                using (eSTS_StagEntities dbContext = new Database.eSTS_StagEntities())
                {
                    ...
                    if (Convert.ToInt32(Session["PortLocation"]) == 0)
                        e.DataSource.Where = "it.[SupplyMethodID] = @pSupplyMethodID and (it.[FlowActionStatusID]=@pflowPendingApproval or it.[FlowActionStatusID]=@pflowPendingRejected)";
                    else
                    {
        ...
        protected void dsActiveApp_Selecting(object sender, EntityDataSourceSelectingEventArgs e)
        {
            ...
            try
            {
                using (eSTS_StagEntities dbContext = new Database.eSTS_StagEntities())
                {
                    ...
                    e.DataSource.WhereParameters.Clear();
                    if (Convert.ToInt32(Session["PortLocation"]) == 0)
                        e.DataSource.Where = "it.[SupplyMethodID] = @pSupplyMethodID and (it.[FlowActionStatusID] = @pPermitIssued or it.[FlowActionStatusID] = @pPendingBL or it.[FlowActionStatusID] = @pPendingPayment or it.[FlowActionStatusID] = @pflowProcessInv)";
                    else
                    {
                        if (Session["PermitIssuerID"].ToString() != "")
                            e.DataSource.Where = "it.[SupplyMethodID]=@pSupplyMethodID and it.[Location]=@pLocation and it.[PermitIssuerID]=@pPermitIssuerID and (it.[FlowActionStatusID] = @pPermitIssued or it.[FlowActionStatusID] = @pPendingBL or it.[FlowActionStatusID] = @pPendingPayment or it.[FlowActionStatusID] = @pflowProcessInv)";
                        else
                            e.DataSource.Where = "it.[SupplyMethodID]=@pSupplyMethodID and it.[Location]=@pLocation and (it.[FlowActionStatusID] = @pPermitIssued or it.[FlowActionStatusID] = @pPendingBL or it.[FlowActionStatusID] = @pPendingPayment or it.[FlowActionStatusID] = @pflowProcessInv)";
                    }
                    e.DataSource.WhereParameters.Add("pLocation", TypeCode.Int32, Session["PortLocation"].ToString());
                    e.DataSource.WhereParameters.Add("pSupplyMethodID", DbType.Guid, supplyMethodID);
                    e.DataSource.WhereParameters.Add("pPermitIssuerID", DbType.Guid, Session["PermitIssuerID"].ToString());
