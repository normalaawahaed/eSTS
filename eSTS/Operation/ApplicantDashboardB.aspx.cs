...
{
    public partial class ApplicantDashboardB : System.Web.UI.Page
    {
        ...
        protected void lilView_Init(object sender, EventArgs e)
        {
            ...
            {
                contentUrl = string.Format("OpAppB.aspx?mode=e&appr=0&adm=0&sno={0}", DataBinder.Eval(container.DataItem, "OperationAppID"));
        ...
        protected void lilCM_Init(object sender, EventArgs e)
        {
            ...
            contentUrl = string.Format("UploadCMBL_B.aspx?mode=e&m=b&appr=0&adm=0&sno={0}", DataBinder.Eval(container.DataItem, "OperationAppID"));
        ...
        protected void lilAmend_Init(object sender, EventArgs e)
        {
            ...
            */
            ...
            if (isPayment == true && isAmend == false && isAmendApprove==false)
            {
                ...
                {
                    contentUrl = string.Format("OpAppB.aspx?mode=a&appr=0&adm=0&sno={0}", DataBinder.Eval(container.DataItem, "OperationAppID"));
        ...
        protected void dsOperationApp_Selecting(object sender, EntityDataSourceSelectingEventArgs e)
        {
            using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
            {
                ...
                e.DataSource.WhereParameters.Add("pSupplyMethodID", DbType.Guid, dbContext.SystemParams.FirstOrDefault<SystemParam>().SupplyMethodB.ToString());
            ...
            try
            {
                using (eSTS_StagEntities dbContext = new Database.eSTS_StagEntities())
                {
                    ...
                    if (Convert.ToInt32(Session["PortLocation"]) == 0)
                        e.DataSource.Where = "it.[CompID]=@pCompID and it.[SupplyMethodID] = @pSupplyMethodID and (it.[FlowActionStatusID] = @pPermitIssued or it.[FlowActionStatusID] = @pPendingBL or it.[FlowActionStatusID] = @pPendingPayment or it.[FlowActionStatusID] = @pflowProcessInv)";
                    else
                    {
                        if (Session["PermitIssuerID"].ToString() != "")
                            e.DataSource.Where = "it.[CompID]=@pCompID and it.[SupplyMethodID]=@pSupplyMethodID and it.[Location]=@pLocation and it.[PermitIssuerID]=@pPermitIssuerID and (it.[FlowActionStatusID] = @pPermitIssued or it.[FlowActionStatusID] = @pPendingBL or it.[FlowActionStatusID] = @pPendingPayment or it.[FlowActionStatusID] = @pflowProcessInv)";
                        else
                            e.DataSource.Where = "it.[CompID]=@pCompID and it.[SupplyMethodID]=@pSupplyMethodID and it.[Location]=@pLocation and (it.[FlowActionStatusID] = @pPermitIssued or it.[FlowActionStatusID] = @pPendingBL or it.[FlowActionStatusID] = @pPendingPayment or it.[FlowActionStatusID] = @pflowProcessInv)";
                    }
                    e.DataSource.WhereParameters.Add("pCompID", DbType.String, Session["CompID"].ToString());
