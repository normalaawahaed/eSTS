        ...
        protected void dsComplete_Selecting(object sender, EntityDataSourceSelectingEventArgs e)
        {
            try
            {
                using (eSTS_StagEntities dbContext = new Database.eSTS_StagEntities())
                {
                    ...
                    string supplyMethodID = "";
                    if (cbMethod.Value.ToString() == "1")
                        supplyMethodID = sysParam.SupplyMethodA.ToString();
                    else if (cbMethod.Value.ToString() == "2")
                        supplyMethodID = sysParam.SupplyMethodB.ToString();

                    string filter = "";

                    if (cbMethod.Value.ToString() == "0")
                        filter = @"  (it.[EstOperationDateTime] >= @pDateFrom and it.[EstOperationDateTime]<= @pDateTo) ";
                    else
                        filter = @" it.[SupplyMethodID] = @pSupplyMethodID and  (it.[EstOperationDateTime] >= @pDateFrom and it.[EstOperationDateTime]<= @pDateTo) ";

                    e.DataSource.Where = filter;
                    ...
                    ////string supplyMethodID = cbMethod.Value.ToString();

                    //e.DataSource.WhereParameters.Clear();
                    //if (Convert.ToInt32(Session["PortLocation"]) == 0)
                    //    e.DataSource.Where = "it.[CompID]=@pCompID and it.[SupplyMethodID] = @pSupplyMethodID";
                    //else
                    //{
