...
{
    public partial class OpAppB : System.Web.UI.Page
    {
            ...
            try
            {
                using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
                {
                    ...
                    else if (Session["mode"].ToString() == "e" || Session["mode"].ToString() == "v" || Session["mode"].ToString() == "p" || Session["mode"].ToString() == "a" || Session["mode"].ToString() == "c")
                    {
                        ...
                        //FSU
                        cbFSU.Value = item.VSID;
                        txtFSUCallSign.Text = item.VSCallSign;
                        txtLatDegree.Text = item.VSLatDegree.ToString();
                        txtLatMin.Text = item.VSLatMin.ToString();
                        txtLongDegree.Text = item.VSLongDegree.ToString();
                        txtLongMin.Text = item.VSLongMin.ToString();
                        txtSupName.Text = item.VSSupritendentName.ToString();
                        txtSupTelNo.Text = item.VSSupritendentTelNo.ToString();

                        //Vessel Receiver
                        txtVesselName.Text = item.VRName;
                        txtPortReg.Text = item.VRPortReg;
                        txtIMONo.Text = item.VRIMONo;
                        cbFlag.Value = item.VRFlag;
                        txtNRT.Text = item.VRNRT.ToString();
                        txtGRT.Text = item.VRGRT.ToString();
                        txtLOA.Text = item.VRLOA.ToString();
                        txtMMSINo.Text = item.VRMMSINo;
                        txtCallSign.Text = item.VRCallSign;
                        cbLastPort.Value = item.VRLastPort;
                        cbNextPort.Value = item.VRNextPort;
        ...
        private void loadTimeline(Guid ApplicationID)
        {
            try
            {
                using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
                {
                    ...
                    {
                        if (flow.IsHideFlow == false || (flow.IsHideFlow == true && flow.ActionDate == null && flow.FlowActionStatusID != sysParam.FlowPendingVerifyDec))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    sb.Append("<dl class='row mb-0'>");
                                    if (flow.FlowActionStatusID == sysParam.FlowReject || flow.FlowActionStatusID == sysParam.FlowAmendRejected)
            ...
            try
            {
                if (SubmitAmendments())
                {
                    if (Session["isApprover"].ToString() == "0")
                        ...
                        Response.Redirect("~//Operation/ApprovalDashboardB.aspx", false);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
                {
                    ...
                    if (item.IsAmend == true)
                    {
                        ...
                        string directory = Server.MapPath("Upload/" + item.CompID + "/" + item.OperationAppID.ToString());
                        string fName = "Permit_" + DateTime.Now.ToString("yyyyMMddHHmmssffff");
                        string PermitQRPath = "~/Operation/Upload/" + item.CompID + "/" + item.OperationAppID.ToString() + "/" + "qrcode_" + fName + ".jpg";

                        item.PermitDocLink = objOperationApp.GenerateQRCode(item.OperationAppID.ToString(), item.CompID, directory, fName, Server.MapPath(PermitQRPath)); //GeneratePermit(operationAppID.ToString(),item.CompID, item.RefID,ref QRPhysicalPath);
                        item.PermitQRCode = Server.MapPath(PermitQRPath);
                    ...
                    else
                    {
                        ...
                        //Notis Permit
                        folderDirectory = Server.MapPath("Upload/" + item.CompID + "/" + item.OperationAppID.ToString());
                        fileName = "NotisPermit_" + DateTime.Now.ToString("yyyyMMddHHmmssffff");
                        PermitQRFilePath = "~/Operation/Upload/" + item.CompID + "/" + item.OperationAppID.ToString() + "/" + "qrcode_" + fileName + ".jpg";

                        item.NPDocLink = objOperationApp.GenerateQRCode(item.OperationAppID.ToString(), item.CompID, folderDirectory, fileName, Server.MapPath(PermitQRFilePath)); //GeneratePermit(operationAppID.ToString(),item.CompID, item.RefID,ref QRPhysicalPath);
                        item.NPQRCode = Server.MapPath(PermitQRFilePath);
                        GeneratePermit(item.OperationAppID.ToString(), item.CompID, item.NPDocLink);
                        ...
                        item.Lampiran1DocLink = objOperationApp.GenerateQRCode(item.OperationAppID.ToString(), item.CompID, folderDirectory, fileName, Server.MapPath(PermitQRFilePath)); //GeneratePermit(operationAppID.ToString(),item.CompID, item.RefID,ref QRPhysicalPath);
                        item.Lampiran1QRCode = Server.MapPath(PermitQRFilePath);
                        GenerateLampiran(item.OperationAppID.ToString(), item.CompID, item.Lampiran1DocLink);
        ...
        protected void btnReject_Click(object sender, EventArgs e)
        {
            try
            {
                using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
                {
                    ...
                    //item.RejectRemark = txtReject.Text;
                    //dbContext.SaveChanges();
                    if (item.IsAmend == true)
                    {
            ...
            try
            {
                ...
                using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
                {
                    ...
                    if (isDraft == true) //Save As Draft
                    {
                        ...
                        if (cbFSU.Value != null)
                        {
                            item.VSID = licCompVesselID;
                            item.VSIMONo = FSU.IMONo;
                            item.VSName = FSU.ShipName;
                            item.VSFlag = FSU.ShipFlag;
                            item.VSPortReg = FSU.PortReg;
                            item.VSCallSign = txtFSUCallSign.Text;
                            item.VSLOA = Convert.ToDouble(FSU.LOA);
                            item.VSGRT = Convert.ToDouble(FSU.GRT);
                            item.VSNRT = Convert.ToDouble(FSU.NRT);
                            item.VSMMSINo = FSU.MMSINo;

                            //FSU
                            item.VRCallSign = txtCallSign.Text;
                            item.VSLatDegree = Convert.ToInt32(txtLatDegree.Text);
                            item.VSLatMin = Convert.ToDecimal(txtLatMin.Text);
                            item.VSLongDegree = Convert.ToInt32(txtLongDegree.Text);
                            item.VSLongMin = Convert.ToDecimal(txtLongMin.Text);
                            item.VSLatitude = LatLong.FromDegreeDecimalMinutes(Convert.ToInt32(txtLatDegree.Text), Convert.ToDouble(txtLatMin.Text)).ToStringD();
                            item.VSLongitude = LatLong.FromDegreeDecimalMinutes(Convert.ToInt32(txtLongDegree.Text), Convert.ToDouble(txtLongMin.Text)).ToStringD();
                            item.VSSupritendentName = txtSupName.Text;
                            item.VSSupritendentTelNo = txtSupTelNo.Text;
                        }

                        item.CompID = Session["CompID"].ToString();
                        item.SOCompID = cbSTSO.Value.ToString();
                        item.SOLicID = FSU.LicCompanyID;
                        item.SupplyMethodID = sysParam.SupplyMethodB;
                        AssignValue(item);
                        item.IsDraft = true;
                        //Default False;
                        item.IsSubmit = false;
                    ...
                    else
                    {
                       ...
                       // if (!IsValidateSuppDoc(FSU.LicCompanyID.ToString(), FSU.LicCompanyVesselID.ToString()))
                        ...
                        item.SupplyMethodID = new Guid(sysParam.SupplyMethodB.Value.ToString());
                        item.VSID = licCompVesselID;
                        item.VSIMONo = FSU.IMONo;
                        item.VSName = FSU.ShipName;
                        item.VSFlag = FSU.ShipFlag;
                    ...
                    LicCompanyVessel FSUMaster = dbContext.LicCompanyVessels.Find(item.VSID);
            ...
            */
            try
            {
                ...
                using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
                {
                    ...
                    if (!IsValidateSuppDoc(item.SOLicID.ToString(), item.VSID.ToString()))
                        return false;
        ...
        private void AssignValue(OperationApp item)
        {
            ...
            //Vessel Supplier
            if (txtIMONo.Text != "")
                item.VRIMONo = txtIMONo.Text;

            if (txtVesselName.Text != "")
                item.VRName = txtVesselName.Text;

            if (cbFlag.Value != null)
                item.VRFlag = cbFlag.Value.ToString();

            if (txtPortReg.Text != "")
                item.VRPortReg = txtPortReg.Text;

            if (txtLOA.Text != "")
                item.VRLOA = Convert.ToDouble(txtLOA.Text);

            if (txtGRT.Text != "")
                item.VRGRT = Convert.ToDouble(txtGRT.Text);

            if (txtNRT.Text != "")
                item.VRNRT = Convert.ToDouble(txtNRT.Text);

            if (txtMMSINo.Text != "")
                item.VRMMSINo = txtMMSINo.Text;
            ...
            if (cbLastPort.Value != null)
            {
                item.VRLastPort = cbLastPort.Value.ToString();
            }
            if (cbNextPort.Value != null)
            {
                item.VRNextPort = cbNextPort.Value.ToString();
            }
...
<p>Penarikan balik lesen aktiviti pelabuhan; dan<br />Disenarai hitam untuk mohon lesen aktiviti pelabuhan; atau<br />Lain-lain tindakan tatatertib mengikut peraturan Perolehan Kerajaan.<br />Sekiranya terdapat mana-mana individu cuba meminta rasuah daripada saya atau mana-mana individu yang berkaitan dengan syarikat ini sebagai ganjaran mendapatkan sebut harga seperti di atas, maka saya berjanji akan dengan segera melaporkan perbuatan tersebut kepada pejabat Suruhanjaya Rasuah Malaysia(SPRM) atau balai polis yang berhampiran.</p>";
        ...
        protected void GeneratePermit(string operationAppID, string refID, string permitFilePath)
        {
            try
            {
                ...
                oRpt.Load(Server.MapPath("~/Operation/PetrolPermit.rpt"));// = new eBunkering.Operation.PetrolPermit();
