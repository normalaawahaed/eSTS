...
{
    public partial class UploadCMBL : System.Web.UI.Page
    {
        ...
        protected void Page_PreInit(object sender, EventArgs e)
        {
            ...
            //this.MasterPageFile = "~/SiteInspinia.Master";
            //else
            //    this.MasterPageFile = "~/SiteInspiniaMethodB.Master";
        }
        ...
        private void LoadForm()
        {
            try
            {
                using (eSTS_StagEntities dbContext = new eSTS_StagEntities())
                {
                    ...
                    divUploadCM.Visible = true;
                    divUploadBL.Visible = true;
        ...
        protected void btnSaveFileCM_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblErrMsg.Text = "Please select Ullage Report";
        ...
        protected void btnSaveFileBL_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblErrMsg.Text = "Please select  Surveyor Report";
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "script", "<script type='text/javascript'>errorAlert();</script>", false);
                    uploadFileBL.Focus();
                    return;
                }
...
<p>Penarikan balik lesen aktiviti pelabuhan; dan<br />Disenarai hitam untuk mohon lesen aktiviti pelabuhan; atau<br />Lain-lain tindakan tatatertib mengikut peraturan Perolehan Kerajaan.<br />Sekiranya terdapat mana-mana individu cuba meminta rasuah daripada saya atau mana-mana individu yang berkaitan dengan syarikat ini sebagai ganjaran mendapatkan sebut harga seperti di atas, maka saya berjanji akan dengan segera melaporkan perbuatan tersebut kepada pejabat Suruhanjaya Rasuah Malaysia(SPRM) atau balai polis yang berhampiran.</p>";
                        ...
                        v_Integration integ = dbContext.v_Integration.Where(w => w.OperationAppID == item.OperationAppID).FirstOrDefault<v_Integration>();
                        ...
                        if (activeFlow.FlowActionStatusID == sysParam.FlowPendingCM || activeFlow.FlowActionStatusID == sysParam.FlowRejectDec)
                        {
                            activeFlow.ActionBy = Session["UserID"].ToString();
                            activeFlow.ActionDate = DateTime.Now;
                            activeFlow.IsActive = false;
                            ...
                            OperationAppFlow appFlowPendingVerify = new OperationAppFlow();
                            appFlowPendingVerify.OperationAppFlowID = Guid.NewGuid();
                            appFlowPendingVerify.OperationAppID = item.OperationAppID;
                            appFlowPendingVerify.FlowActionStatusID = sysParam.FlowPendingVerifyDec;
                            appFlowPendingVerify.IsActive = false;
                            appFlowPendingVerify.CreatedBy = Session["UserID"].ToString();
                            appFlowPendingVerify.CreatedDate = DateTime.Now.AddSeconds(2);
                            appFlowPendingVerify.ActionBy = "elpjt";

                            dbContext.OperationAppFlows.Add(appFlowPendingVerify);
                            dbContext.SaveChanges(Session["UserID"].ToString(), "OperationAppFlowID", item.OperationAppID);

                            //Create Pending Approval
                            OperationAppFlow appFlowApproval = new OperationAppFlow();
                            appFlowApproval.OperationAppFlowID = Guid.NewGuid();
                            appFlowApproval.OperationAppID = item.OperationAppID;
                            appFlowApproval.FlowActionStatusID = sysParam.FlowPendingApproveDec;
                            appFlowApproval.IsActive = true;
                            appFlowApproval.CreatedBy = Session["UserID"].ToString();
                            appFlowApproval.CreatedDate = DateTime.Now.AddSeconds(3);
                            appFlowApproval.ActionBy = "elpjt";

                            dbContext.OperationAppFlows.Add(appFlowApproval);
                            dbContext.SaveChanges(Session["UserID"].ToString(), "OperationAppFlowID", item.OperationAppID);
        ...
        private bool isValidate(OperationApp item)
        {
            try
            {
                ...
                if ((item.CMAttachLink == "" || item.CMAttachLink == null) && (item.BLAttachLink == ""|| item.BLAttachLink == null))
                {
                    ...
                    lblErrMsg.Text = "Please attach Ullage Report / Surveyor Report";
                    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "script", "<script type='text/javascript'>errorAlert();</script>", false);
                    txtCMNo.Focus();
                    return false;
        ...
        private void PushToPJ(v_Integration item)
        {
            try
            {
                ...
                using (SqlConnection sqlConn = new SqlConnection(conns))
                {
                    ...
                    //Check record exist
                    string querystring = " Select COUNT(*) from  [sts_transaction] where OperationAppID='" + item.OperationAppID.ToString() + "'";
                    SqlCommand cmd1 = new SqlCommand(querystring, sqlConn);
                    sqlConn.Open();
                    int recordExist = (int)cmd1.ExecuteScalar();

                    string sqlCmd = "";
                    if (recordExist == 1) //Update
                    {
                        sqlCmd = @"UPDATE  [sts_transaction] SET 
                                  ETA=@ETA, ETD=@ETD, ActOilMT=@ActOilMT, CMAttachLink=@CMAttachLink, BLAttachLink=@BLAttachLink, isProcess=0
                                   WHERE OperationAppID=@OperationAppID";
                    }
                    else //Insert
                    {
                        sqlCmd = @"INSERT INTO  [sts_transaction]
                                      (AgentID, AgentName, CompID, CompanyName, OperationAppID, FSU, FSUIMONo, FSULastPort, FSUNextPort, VesselName, VesselIMONo, VesselLastPort, VesselNextPort, ETA, ETD
                                    , ActOilMT,CargoType, UOMCode, UOMDesc, CMAttachLink, BLAttachLink,sts_casenum)
                                     VALUES
                                     (@AgentID, @AgentName, @CompID, @CompanyName, @OperationAppID, @FSU, @FSUIMONo, @FSULastPort, @FSUNextPort, @VesselName, @VesselIMONo, @VesselLastPort, @VesselNextPort, @ETA, @ETD
                                    , @ActOilMT,@CargoType, @UOMCode, @UOMDesc, @CMAttachLink, @BLAttachLink,@sts_casenum)";
                    }
                    ...
                    {
                        cmd.Parameters.AddWithValue("@sts_casenum", item.CaseNum);
                        cmd.Parameters.AddWithValue("@AgentID", item.AgentID);
                        cmd.Parameters.AddWithValue("@AgentName", item.AgentName);
                        cmd.Parameters.AddWithValue("@CompID", item.CompID);
                        cmd.Parameters.AddWithValue("@CompanyName", item.CompanyName);
                        cmd.Parameters.AddWithValue("@OperationAppID", item.OperationAppID);
                        cmd.Parameters.AddWithValue("@FSU", item.FSU);
                        cmd.Parameters.AddWithValue("@FSUIMONo", item.FSUIMONo);
                        if (item.FSULastPort != null)
                            cmd.Parameters.AddWithValue("@FSULastPort", item.FSULastPort);
                        else
                            cmd.Parameters.AddWithValue("@FSULastPort", "");
                        if (item.FSUNextPort != null)
                            cmd.Parameters.AddWithValue("@FSUNextPort", item.FSUNextPort);
                        else
                            cmd.Parameters.AddWithValue("@FSUNextPort", "");
                        cmd.Parameters.AddWithValue("@VesselName", item.VesselName);
                        cmd.Parameters.AddWithValue("@VesselIMONo", item.VesselIMONo);
                        cmd.Parameters.AddWithValue("@VesselLastPort", item.VesselLastPort);
                        cmd.Parameters.AddWithValue("@VesselNextPort", item.VesselNextPort);
                        cmd.Parameters.AddWithValue("@ETA", item.ETA);
                        cmd.Parameters.AddWithValue("@ETD", item.ETD);
                        cmd.Parameters.AddWithValue("@CargoType", item.OilTypeDesc);
                        cmd.Parameters.AddWithValue("@ActOilMT", item.ActOilMT);
                        cmd.Parameters.AddWithValue("@UOMCode", item.UOMCode);
                        cmd.Parameters.AddWithValue("@UOMDesc", item.UOMDesc);
                        if (item.CMAttachLink != null)
                            cmd.Parameters.AddWithValue("@CMAttachLink", item.CMAttachLink);
                        else
                            cmd.Parameters.AddWithValue("@CMAttachLink", "");

                        if (item.BLAttachLink != null)
                            cmd.Parameters.AddWithValue("@BLAttachLink", item.BLAttachLink);
                        else
                            cmd.Parameters.AddWithValue("@BLAttachLink", ""); 
